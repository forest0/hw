...
void RenderingWidget::mouseMoveEvent(QMouseEvent *e)
{
    ...
    case Qt::MidButton:
        eye_goal_[0] -= 
            (4.0*GLfloat(e->x() - current_position_.x())) 
                / GLfloat(width());
        eye_goal_[1] += 
            (4.0*GLfloat(e->y() - current_position_.y())) 
                / GLfloat(height());
        current_position_ = e->pos();
        break;
    default:
...
void RenderingWidget::SetBackground()
{
    QColor color = QColorDialog::getColor(
            Qt::white, this, tr("background color"));
...
void RenderingWidget::ReadMesh()
{
    ...
    emit(operatorInfo(QString("Read Mesh from") 
                + filename + QString(" Done")));
    emit(meshInfo(ptr_mesh_->num_of_vertex_list(),
                ptr_mesh_->num_of_edge_list(),
                ptr_mesh_->num_of_face_list()));
    updateGL();
}
...
void RenderingWidget::WriteMesh()
{
    ...
    emit(operatorInfo(QString("Write Mesh to ") 
                + filename + QString(" Done")));
}
...
void RenderingWidget::LoadTexture()
{
    ...
    {
        //        QMessageBox::warning(this, tr("Load Fialed!"), 
        //        tr("Cannot Load Image %1").arg(filenames.at(0)));
        emit(operatorInfo(QString("Load Texture Failed!")));
        return;
        /*
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, tex1.width(), 
            tex1.height(),
        GL_RGBA, GL_UNSIGNED_BYTE, tex1.bits());

    is_load_texture_ = true;
    emit(operatorInfo(QString("Load Texture from ") 
                + filename + QString(" Done")));
}
...
void RenderingWidget::DrawPoints(bool bv)
{
    ...
    const std::vector<HE_vert*>& verts = 
        *(ptr_mesh_->get_vertex_list());
    glBegin(GL_POINTS);
